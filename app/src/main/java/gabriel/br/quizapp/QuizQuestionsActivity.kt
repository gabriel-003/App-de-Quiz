...
class QuizQuestionsActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private var aSelectedOption: Int = 0
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        aQuestionsList = Constants.getQuestions()
    ...
    private fun setQuestion() {
        defaultOptionsView()
        val question: Question = aQuestionsList!![aCurrentPosition - 1]
        ivImage?.setImageResource(question.image)
        progressBar?.progress = aCurrentPosition
        tvProgress?.text = "$aCurrentPosition / ${progressBar?.max}"
    ...
    private fun selectedOptionView(tv: TextView, selectedOptionInt: Int){
        defaultOptionsView()

        aSelectedOption = selectedOptionInt
        ...
        when(view?.id){
            ...
            R.id.btn_submit -> {
                if(aSelectedOption == 0){
                    ...
                    if(question!!.correctAnswer != aSelectedOption){
                        answerView(aSelectedOption, R.drawable.wrong_option_border_bg)
                    }else{
                        aCorrectAnswers++
                    }
                    ...
                    if(aCurrentPosition == aQuestionsList!!.size){
                        ...
                        btnSubmit?.text = "IR PARA A PRÓXIMA PERGUNTA"
                    }
    ...
    private fun answerView(answer: Int, drawableView: Int){
        when(answer){ //pode usar when quando for int. Ex: asnwer, id
            1 -> {
